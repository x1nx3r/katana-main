...
class softaculous_pclzip extends PclZip{
...
//    $v_path = "./";
  ...
  function _privAdd($p_filedescr_list, &$p_result_list, &$p_options)
  {
	...
	global $softaculous_tempheader;
	
	$softaculous_tempheader = '';
	
	$tempheaderfile = PCLZIP_TEMPORARY_DIR.uniqid('softaculous').'.tmp';
	if(($softaculous_tempheader = @fopen($tempheaderfile, 'wb')) == 0){
  ...
  function _privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
  {
	...
	global $_softaculous_result_list, $_softaculous_nb;
	
    $_softaculous_result_list = array();
	$_softaculous_nb = 0;	
  ...
  function _privFileDescrExpand(&$p_filedescr_list, &$p_options)
  {
    ...
    for ($i=0; $i<sizeof($p_filedescr_list); $i++) {
      ...
      // ----- Missing file
		...
		$softaculous_error['read'] = lang_vars($softaculous_lang['could_not_read'], array($v_descr['filename']));
		// ----- Error log
...
//          || (   is_dir($p_filedescr_list[$j]['filename'])
      if (   ($_file['type'] == 'file')
        ...
        //$_softaculous_result_list[$_softaculous_nb++] = $v_header;
  ...
  function _privWriteCentralFileHeader(&$p_header)
  {
    ...
    fputs($softaculous_tempheader, $v_binary_data, 46);

    // ----- Write the variable fields
    if (strlen($p_header['stored_filename']) != 0)
    {
      fputs($softaculous_tempheader, $p_header['stored_filename'], strlen($p_header['stored_filename']));
    }
    if ($p_header['extra_len'] != 0)
    {
      fputs($softaculous_tempheader, $p_header['extra'], $p_header['extra_len']);
    }
    if ($p_header['comment_len'] != 0)
    {
      fputs($softaculous_tempheader, $p_header['comment'], $p_header['comment_len']);
    }
