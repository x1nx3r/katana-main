...
class Products extends Posts {
	...
	protected $tags = [
		...
		'{{{stockStatusClass}}}',
		'{{{stockQuantity}}}',
		'{{product_catToStr}}', // converts the array of categories to this form cat1, cat2, cat3
		'{{product_tagToStr}}'  // converts the array of tags to this form tag1, tag2, tag3
	];
	...
	public function getRecordsWithTags( array $args = [] ){
		...
		if ( $availableRecords && $availableRecords->have_posts() ) {
			...
			foreach( $availableRecords->posts as $post ) {
				...
				$attachment_ids = $product->get_gallery_image_ids();
				if( !empty( $attachment_ids[0] ) ){
					$secondaryImage = $attachment_ids[0];
				}
				...
				$postInfo = [
					...
					'{{{stockStatusClass}}}' => $product->is_in_stock() ? 'in-stock' : 'out-of-stock',
					'{{{stockQuantity}}}' => $product->get_stock_quantity(),
					'{{product_catToStr}}' => 	$this->getTaxonomyTermsStr( $post->ID, 'product_cat' ),
					'{{product_tagToStr}}' => $this->getTaxonomyTermsStr( $post->ID, 'product_tag' ),
				];
